...
const FsState = struct {
    ...
    fn stopWorkers(self: *FsState) void {
        ...
        // A worker could be between checking shutdown and calling cond.wait;
        // broadcasting without the lock risks the signal being lost.
        {
            self.repl_log.mutex.lock();
            defer self.repl_log.mutex.unlock();
            self.repl_log.cond.broadcast();
...
fn doUnmount(mountpoint: []const u8) void {
    ...
    switch (result) {
        ...
        .Signal => |sig| {
            std.debug.print("{s} killed by signal {d}\n", .{ cmd, sig });
            std.process.exit(1);
        },
        .Stopped => |sig| {
            std.debug.print("{s} stopped by signal {d}\n", .{ cmd, sig });
            std.process.exit(1);
        },
        .Unknown => |val| {
